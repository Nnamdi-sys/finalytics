use crate::data::yahoo::config::{
    Fundamentals, FundamentalsResponse, Object, StatementFrequency, StatementType,
};
...
pub async fn get_fundamentals(
    ...
    let _type = match statement_type {
        ...
        StatementType::FinancialRatios => {
            return Err(format!(
                "Statement type 'FinancialRatios' is not supported for get_fundamentals (ticker '{symbol}')"
            ).into());
        }
    };
    ...
    let result = get_json_response(url).await.map_err(|e| {
        format!(
            "fetching fundamentals for ticker '{symbol}' ({statement_type:?}, {frequency:?}): {e}"
        )
    })?;
    let data: FundamentalsResponse = serde_json::from_value(result).map_err(|e| {
        format!("Failed to deserialize fundamentals response for ticker '{symbol}': {e}")
    })?;
        ...
        for (key, value) in item {
            if _type_clone.contains(key.as_str()) {
                let items: Vec<Object> = serde_json::from_value(value.to_string().parse()?)
                    .map_err(|e| format!("Failed to deserialize fundamentals item '{key}' for ticker '{symbol}': {e}"))?;
                let date_vec = items
                    .iter()
                    .map(|x| x.asOfDate.clone())
                    .collect::<Vec<String>>();
                if date_vec.len() < 4 {
                ...
                if items.len() == columns[0].len() {
                    let vars_vec = items
                        ...
                        .collect::<Vec<f64>>();
                    let vars_series = Column::new(
                        key.as_str().replace(&frequency.to_string(), "").into(),
                        &vars_vec,
                    );
                    columns.push(vars_series);
                } else {
                    let mut vars_vec: Vec<f64> = vec![];
                    for d in columns[0].as_materialized_series().iter() {
                        let mut found = false;
...
pub async fn financial_ratios(
    ...
    let income_statement =
        get_fundamentals(symbol, StatementType::IncomeStatement, frequency).await?;
    ...
    let mut df = df
        .drop("date")
        .map_err(|e| format!("Failed to drop 'date' column: {e}"))?;
...
pub async fn cashflow_statement(
    symbol: &str,
    frequency: StatementFrequency,
    formatted: Option<bool>,
) -> Result<DataFrame, Box<dyn Error>> {
